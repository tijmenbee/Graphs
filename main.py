...
#print(f"louvains: {louvains}")
#print(f"louvain time: {full_louvain_time}")
exit()
partition_orig_next, time_orig = eval_time_com(partial(community_louvain.best_partition, G, partition_orig_next, random_state=42))
...
for nodes in truncated_dict.values():
        break
        for edge in nodes:
            for node in edge:
                if node not in partition_next:
                    partition_next[node] = -1
                if node not in partition_orig_next:
                    partition_orig_next[node] = -1

    #partition,_  = extended_louvain(G,partition, truncated_dict, step_size/10)
